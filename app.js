...
async function openGameProfile(slug){
  ...
  document.getElementById('gameProfile').innerHTML=`
    ...
    <div class="game-profile-body">
      ...
      <section class="game-reviews"><h3>Отзывы</h3>${own?`<div class="review-done">Вы уже поставили оценку игре</div>`:played?`<div class="review-form"><div class="review-stars">${[1,2,3,4,5].map(n=>`<button onclick="selectReviewRating(${n})">★</button>`).join('')}</div><textarea id="reviewComment" maxlength="500" placeholder="Поле ввода"></textarea><button class="btn btn-primary" onclick="submitReview('${slug}')">Отправить</button></div>`:`<div class="review-locked">Сыграйте в игру, чтобы поставить оценку</div>`}<div class="review-list">${comments||'<div class="review-empty">Комментариев пока нет</div>'}</div></section>
    </div>`;
